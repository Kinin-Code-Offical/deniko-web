...
describe('ESLint i18n Rules', () => {
  ...
  it('should flag hardcoded text in JSX', async () => {
    ...
    const results = await eslint.lintText(code, { filePath: 'app/test-hardcoded.tsx' });
    const {messages} = results[0];

    const hardcodedErrors = messages.filter(m => m.ruleId === 'no-restricted-syntax');
    
    // Expect errors for <h1> text, <p> text, and placeholder
    expect(hardcodedErrors.length).toBeGreaterThanOrEqual(3);
    expect(hardcodedErrors.some(m => m.message.includes('Avoid hardcoded text'))).toBe(true);
    expect(hardcodedErrors.some(m => m.message.includes('Avoid hardcoded placeholder'))).toBe(true);
  });
...
export default function I18n({ dict }: { dict: Dictionary }) {
    ...
    const results = await eslint.lintText(code, { filePath: 'app/test-i18n.tsx' });
    const {messages} = results[0];
