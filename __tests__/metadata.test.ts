...
export function validateMetadata(metadata: Metadata | undefined) {
    ...
    expect(metadata?.description, 'Metadata description is required').toBeDefined()
    ...
    if (typeof metadata?.title === 'object' && metadata?.title !== null) {
        ...
        expect(title.default, 'Default title is required when using template').toBeDefined()
    } else {
        expect(typeof metadata?.title, 'Title should be a string or template object').toBe('string')
        expect((metadata?.title as string).length).toBeGreaterThan(0)
    }

    expect(typeof metadata?.description, 'Description should be a string').toBe('string')
