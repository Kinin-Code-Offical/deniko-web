...
export function scanFile(filePath: string, sourceCode?: string): Issue[] {
  ...
  const isI18nCall = (node: ts.Node): boolean => {
    if (ts.isCallExpression(node)) {
      ...
      if (ts.isIdentifier(expression)) {
        return ['t', 'dict', 'cva', 'cn', 'clsx', 'twMerge', 'tv', 'signIn', 'signOut'].includes(expression.text);
      }
      if (ts.isPropertyAccessExpression(expression) && (ts.isIdentifier(expression.expression) && (expression.expression.text === 'console' || expression.expression.text === 'logger'))) {
        return true;
      }
    }
    return false;
  ...
  const ignoredValues = new Set<string>([
    ...
    'canonical', 'alternate', 'author', 'description', 'keywords', 'robots',
    ...
    'summary', 'summary_large_image', 'app', 'website', 'article', 'profile',
    'index', 'follow', 'noindex', 'nofollow',
    'latin', 'cyrillic', 'greek',
    'class', 'style', 'id', 'for', 'name', 'type', 'href', 'src', 'alt', 'title', 'placeholder', 'value', 'key', 'target', 'rel', 'asChild',
    'width', 'height', 'loading', 'decoding', 'fetchPriority', 'crossOrigin', 'referrerPolicy',
    'auto', 'yes', 'no', 'on', 'off', 'true', 'false',
    'button', 'submit', 'reset', 'checkbox', 'radio', 'text', 'password', 'email', 'number', 'tel', 'url', 'search', 'date', 'time', 'datetime-local', 'file', 'hidden', 'image', 'range', 'color',
    'use client', 'use server',
    'light', 'dark', 'system',
    'default', 'outline', 'secondary', 'ghost', 'link', 'destructive',
    'sm', 'md', 'lg', 'xl', '2xl', '3xl', '4xl', '5xl', '6xl', '7xl', 'full', 'screen', 'min', 'max', 'fit',
    ...
    'pointer', 'wait', 'text', 'move', 'help', 'not-allowed',
    'resize', 'resize-x', 'resize-y', 'resize-none',
    'border-box', 'content-box',
    'absolute', 'relative', 'fixed', 'sticky', 'static',
    'block', 'inline-block', 'inline', 'flex', 'inline-flex', 'grid', 'inline-grid',
    'table', 'inline-table', 'table-row', 'table-cell', 'table-column', 'table-column-group', 'table-footer-group', 'table-header-group', 'table-row-group', 'table-caption', 'list-item',
    'latin', '--font-geist-sans', '--font-geist-mono',
    ...
    'student_not_found', 'student_unlinked', 'shadow_student_deleted', 'failed_to_delete_student',
    'relation_not_found', 'cannot_delete_claimed', 'invite_regenerated', 'failed_to_regenerate_token',
    'invite_opened', 'invite_closed', 'failed_to_toggle_invite', 'customName', 'privateNotes',
    'firstName', 'lastName', 'gradeLevel', 'phone', 'parentName', 'parentPhone', 'parentEmail',
    'avatarUrl', 'info_updated', 'failed_to_update_settings', 'no_relation_found',
    'hex', 'confirmPassword', 'verifyEmail', 'resendVerificationEmailAction', 'forgotPassword', 'resetPassword',
    'registerUser', 'completeOnboarding', 'createStudent', 'claimStudentProfile', 'getInviteDetails',
    'getStudentProfileByToken', 'updateStudent', 'unlinkStudent', 'deleteStudent', 'updateStudentRelation',
    'deleteShadowStudent', 'regenerateInviteToken', 'toggleInviteLink', 'updateStudentSettings',
    'Registration Error', 'Verification Error', 'Resend Error', 'Forgot Password Error', 'Reset Password Error',
    'Onboarding Error', 'Unauthorized attempt', 'Invalid fields', 'Failed to upload avatar',
    'Student created successfully', 'Failed to create student', 'Failed to claim profile',
    'Failed to fetch invite details', 'Failed to fetch profile by token', 'Failed to update student',
    'Failed to archive student', 'Failed to delete student', 'Failed to regenerate token',
    'Failed to toggle invite link', 'Failed to update student settings', 'Resend Verification Error',
    ...
    'PPP p',
    'cookie-consent',
    'imamge/jpeg', 'image/jpeg', 'File not found', 'data', 'ok', 'connected', 'disconnected', 'Unknown error',
    'x-nonce', 'daily', 'monthly', 'any', 'Web Browser', '4.8', 'p', '10_min', '2_hours', // ignore-any-check
    'keydown', 'click', 'preserve-3d', 'center bottom', 'easeOut', 'change', 'string', 'lazy',
    'pointer-events-none opacity-50', 'large', 'text-blue-600 dark:text-blue-400',
    'bg-blue-50 dark:bg-blue-900/20', 'text-indigo-600 dark:text-indigo-400',
    'bg-indigo-50 dark:bg-indigo-900/20', 'text-emerald-600 dark:text-emerald-400',
    'bg-emerald-50 dark:bg-emerald-900/20', 'text-purple-600 dark:text-purple-400',
    'bg-purple-50 dark:bg-purple-900/20', 'text-orange-600 dark:text-orange-400',
    'bg-orange-50 dark:bg-orange-900/20', 'text-pink-600 dark:text-pink-400',
    'bg-pink-50 dark:bg-pink-900/20', 'text-cyan-600 dark:text-cyan-400',
  ...
  const visit = (node: ts.Node) => {
    if (ts.isStringLiteral(node)) {
      ...
      // Ignore colors
      if (text.startsWith('#') && (text.length === 4 || text.length === 7 || text.length === 9)) return;

      // Ignore CSS units and values
      if (/^-?\d+(\.\d+)?(px|rem|em|%|vw|vh|s|ms|deg|rad|turn|fr)$/.test(text)) return;
      ...
      if (ts.isPropertyAssignment(parent)) {
        ...
        if (ts.isIdentifier(parent.name) && ['className', 'class', 'variant', 'size', 'href', 'src', 'type', 'id', 'name', 'value', 'key', 'target', 'rel', 'asChild'].includes(parent.name.text)) return;
      }
