...
const IGNORE_DIRS = ['node_modules', '.next', 'dist', 'build', 'coverage', '.git', '.vscode'];
 ...
 */
export function scanFile(filePath: string): { line: number; content: string }[] {
    try {
...
if (isMainModule) {
    ...
    files.forEach(file => {
        ...
        if (file.includes('check-any.ts') || file.includes('check-any.test.ts')) return;
    ...
    console.log('\n--------------------------------------------------');
    if (errorCount > 0) {
        console.error(`❌ Found \x1b[31m${errorCount}\x1b[0m occurrences of 'any' in ${fileCount} files.`);
